...
const HeroTestimonialBadge: React.FC<HeroTestimonialBadgeProps> = ({
  ...
  return (
    <div className="flex items-center justify-center">
      <div className={`relative flex flex-col items-center justify-center gap-1.5 rounded-2xl px-4 py-2 w-full max-w-md mx-auto h-[100px] transform -translate-y-1 transition-all duration-300 ease-in-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
        
        {/* Étoiles TrustPilot style */}
        <div className={`flex gap-0.5 transition-all duration-300 ease-in-out ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
          {[...Array(5)].map((_, i) => (
            <div
              key={i}
        ...
        {/* Commentaire */}
        <p className={`text-sm text-gray-600 max-w-[400px] font-medium leading-tight text-center line-clamp-2 transition-all duration-300 ease-in-out ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
