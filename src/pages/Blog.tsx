      ...
      <div className="container mx-auto py-20 px-4">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {blogPosts.map((post) => (
            <article 
              key={post.id}
              className="group flex flex-col bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-300 border border-sky-100/50 overflow-hidden"
            >
              <div className="p-6 flex-1">
                <time className="text-sm font-medium text-sky-600 bg-sky-50 px-3 py-1 rounded-full inline-block">
                  {format(new Date(post.date), 'dd-MM-yyyy')}
                </time>
                <h2 className="text-xl font-semibold text-gray-800 mt-2 mb-3 group-hover:text-sky-600 transition-colors">
                  {post.title}
                </h2>
