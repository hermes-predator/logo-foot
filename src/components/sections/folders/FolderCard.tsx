...
const FolderCard: React.FC<FolderCardProps> = ({
  ...
  return (
    <div className={`space-y-2 p-4 bg-gradient-to-br ${colors.bg} ${borderClass} transition-all duration-200 ${colors.hoverBg} ${roundedTopClass} ${roundedBottomClass} relative group border border-gray-200/50 shadow-sm`}>
      ...
      <div className={`grid grid-cols-2 gap-3 ${colors.itemBg} rounded-sm p-3 border border-gray-100/60`}>
      ...
      {collections && (
        <div className="mt-1">
          ...
          <p className={`text-xs leading-relaxed text-gray-700 ${colors.itemBg} rounded-sm p-3 border border-gray-100/60`}>
            {collections}
          </p>
        </div>
      )}
